...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_event'])) {
    ...
    if ($deleteId > 0) {
        try {
            ...
            $stmt = $pdo->prepare('DELETE FROM usg_confirm_attendance WHERE event_id = ?');
            $stmt->execute([$deleteId]);
            
            // Delete related records from attendance table
            $stmt = $pdo->prepare('DELETE FROM usg_attendance WHERE event_id = ?');
            $stmt->execute([$deleteId]);
            
            // Now delete the event
        $stmt = $pdo->prepare('DELETE FROM usg_events WHERE id = ?');
        $stmt->execute([$deleteId]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['clear_all_events'])) {
    $stmt = $pdo->prepare('DELETE FROM usg_events');
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['clear_all_attendance'])) {
    $stmt = $pdo->prepare('DELETE FROM usg_attendance');
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['clear_all_payments'])) {
    $stmt = $pdo->prepare('DELETE FROM usg_pay');
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['clear_all_items'])) {
    $stmt = $pdo->prepare('DELETE FROM usg_lst_fnd');
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['clear_all_feedback'])) {
    $stmt = $pdo->prepare('DELETE FROM usg_feedbk');
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['clear_all_announcements'])) {
    $stmt = $pdo->prepare('DELETE FROM usg_announcements');
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_event'])) {
    ...
    if (empty($errors) && $updateId > 0) {
        $stmt = $pdo->prepare('UPDATE usg_events SET eventname = ?, startdate = ?, enddate = ?, description = ? WHERE id = ?');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_event'])) {
    ...
    $description = trim($_POST['eventDescription'] ?? '');
        ...
        $stmt = $pdo->prepare('INSERT INTO usg_events (eventname, startdate, enddate, description) VALUES (?, ?, ?, ?)');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_attendance'])) {
    ...
    if ($deleteId > 0) {
        $stmt = $pdo->prepare('DELETE FROM usg_attendance WHERE id = ?');
        $stmt->execute([$deleteId]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_attendance'])) {
    ...
    // Update if no errors
    if (empty($errors) && $updateId > 0) {
        $stmt = $pdo->prepare('UPDATE usg_attendance SET name = ?, date = ?, time = ?, event_id = ? WHERE id = ?');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_attendance'])) {
    ...
    if (empty($errors)) {
        ...
        $stmt = $pdo->prepare('INSERT INTO usg_confirm_attendance (name, date, time, event_id) VALUES (?, ?, ?, ?)');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['approve_attendance'])) {
    ...
    if ($attendance_id > 0) {
        try {
            ...
            $stmt = $pdo->prepare('SELECT * FROM usg_confirm_attendance WHERE id = ?');
            $stmt->execute([$attendance_id]);
            $attendance = $stmt->fetch();
            
            if ($attendance) {
                // Insert into attendance table
                $stmt = $pdo->prepare('INSERT INTO usg_attendance (name, date, time, event_id) VALUES (?, ?, ?, ?)');
                $stmt->execute([$attendance['name'], $attendance['date'], $attendance['time'], $attendance['event_id']]);
                
                // Delete from confirm_attendance
                $stmt = $pdo->prepare('DELETE FROM usg_confirm_attendance WHERE id = ?');
                $stmt->execute([$attendance_id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_payment'])) {
    ...
    if ($deleteId > 0) {
        $stmt = $pdo->prepare('DELETE FROM usg_pay WHERE pay_id = ?');
        $stmt->execute([$deleteId]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_payment'])) {
    ...
    // Update if no errors
    if (empty($errors) && $updateId > 0) {
        $stmt = $pdo->prepare('UPDATE usg_pay SET payname = ?, amount = ?, pay_startdate = ?, pay_enddate = ?, pay_description = ? WHERE pay_id = ?');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_payment'])) {
    ...
    // Create if no errors
    if (empty($errors)) {
        $stmt = $pdo->prepare('INSERT INTO usg_pay (payname, amount, pay_startdate, pay_enddate, pay_description) VALUES (?, ?, ?, ?, ?)');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_item'])) {
    ...
    if ($deleteId > 0) {
        $stmt = $pdo->prepare('DELETE FROM usg_lst_fnd WHERE lst_id = ?');
        $stmt->execute([$deleteId]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_feedback'])) {
    ...
    if ($deleteId > 0) {
        $stmt = $pdo->prepare('DELETE FROM usg_feedbk WHERE feed_id = ?');
        $stmt->execute([$deleteId]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_item'])) {
    ...
    if (empty($errors) && $updateId > 0) {
        if ($imageData) {
            $stmt = $pdo->prepare('UPDATE usg_lst_fnd SET lst_name = ?, category = ?, date_found = ?, location = ?, description = ?, status = ?, lst_img = ? WHERE lst_id = ?');
            $stmt->execute([$itemName, $category, $dateFound, $location, $description, $status, $imageData, $updateId]);
        } else {
            $stmt = $pdo->prepare('UPDATE usg_lst_fnd SET lst_name = ?, category = ?, date_found = ?, location = ?, description = ?, status = ? WHERE lst_id = ?');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_item'])) {
    ...
    // Create if no errors
    if (empty($errors)) {
        $stmt = $pdo->prepare('INSERT INTO usg_lst_fnd (lst_name, category, date_found, location, description, status, lst_img) VALUES (?, ?, ?, ?, ?, ?, ?)');
...
try {
    // Fetch events
    $stmt   = $pdo->query('SELECT * FROM usg_events ORDER BY startdate DESC');
    $events = $stmt->fetchAll();

    // Fetch attendance with event names
    $stmt = $pdo->query('SELECT a.*, e.eventname FROM usg_attendance a JOIN usg_events e ON a.event_id = e.id ORDER BY a.date DESC, a.time DESC');
    $attendances = $stmt->fetchAll();

    // Fetch confirmed attendance with event names
    $stmt = $pdo->query('SELECT ca.*, e.eventname FROM usg_confirm_attendance ca JOIN usg_events e ON ca.event_id = e.id ORDER BY ca.date DESC, ca.time DESC');
    $confirmedAttendances = $stmt->fetchAll();

    // Fetch payments
    $stmt     = $pdo->query('SELECT * FROM usg_pay ORDER BY pay_startdate DESC');
    $payments = $stmt->fetchAll();

    // Fetch lost and found items
    // First, check if the new columns exist
    $checkColumns = $pdo->query("SHOW COLUMNS FROM usg_lst_fnd LIKE 'date_found'");
    ...
    // Now fetch the items
    $stmt = $pdo->query('SELECT * FROM usg_lst_fnd ORDER BY date_found DESC');
    $lostAndFoundItems = $stmt->fetchAll();
} catch (\PDOException $e) {
    // If there's still an error, try fetching without the new columns
    try {
        $stmt = $pdo->query('SELECT * FROM usg_lst_fnd');
        $lostAndFoundItems = $stmt->fetchAll();
    } catch (\PDOException $e) {
        $lostAndFoundItems = [];
...
if (isset($_GET['edit_id'])) {
    ...
    $stmt   = $pdo->prepare('SELECT * FROM usg_events WHERE id = ?');
    $stmt->execute([$editId]);
...
if (isset($_GET['edit_announcement_id'])) {
    ...
    $stmt = $pdo->prepare('SELECT * FROM usg_announcements WHERE id = ?');
...
if (isset($_GET['edit_att_id'])) {
    ...
    $stmt      = $pdo->prepare('SELECT * FROM usg_attendance WHERE id = ?');
...
if (isset($_GET['edit_pay_id'])) {
    ...
    $stmt      = $pdo->prepare('SELECT * FROM usg_pay WHERE pay_id = ?');
...
if (isset($_GET['edit_item_id'])) {
    ...
    $stmt = $pdo->prepare('SELECT * FROM usg_lst_fnd WHERE lst_id = ?');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_announcement'])) {
    ...
    if ($deleteId > 0) {
        try {
            $stmt = $pdo->prepare('DELETE FROM usg_announcements WHERE id = ?');
            $stmt->execute([$deleteId]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_announcement'])) {
    ...
    if (empty($errors) && $updateId > 0) {
        try {
            $stmt = $pdo->prepare('UPDATE usg_announcements SET title = ?, content = ? WHERE id = ?');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_announcement'])) {
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $pdo->query('SELECT MAX(id) as max_id FROM usg_announcements');
            $result = $stmt->fetch();
            $next_id = ($result['max_id'] ?? 0) + 1;

            // Insert the announcement
            $stmt = $pdo->prepare('INSERT INTO usg_announcements (id, title, content, created_at) VALUES (?, ?, ?, CURDATE())');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['clear_all_approvals'])) {
    $stmt = $pdo->prepare('DELETE FROM usg_confirm_attendance');
    $stmt->execute();
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <nav id="sidebar" class="col-md-3 col-lg-2 d-md-block sidebar" aria-label="Sidebar navigation" style="background: linear-gradient(135deg, #211948 0%, #232526 100%);">
                ...
                <div class="position-sticky pt-3">
                    <ul class="nav flex-column">
                        ...
                        <!-- Announcement -->
                        <li class="nav-item fw-bold">
                            <a class="nav-link" data-bs-toggle="collapse" href="#announcementsSubMenu" role="button" aria-expanded="true" aria-controls="announcementsSubMenu">
                            ...
                            <div class="collapse" id="announcementsSubMenu">
                                <ul class="nav flex-column ps-3">
                                    <li class="nav-item fw-bold">
                                        <a class="nav-link" href="#" data-section="createAnnouncementSection" id="navCreateAnnouncement">
                                            <i class="bi bi-plus-circle me-2"></i>
                                            Create Announcement
                                        </a>
                                    </li>
                                    <li class="nav-item fw-bold">
                                        <a class="nav-link" href="#" data-section="viewAnnouncementsSection" id="navViewAnnouncements">
            ...
            <main id="content" class="col-md-9 ms-sm-auto col-lg-10 px-md-4 main-content" role="main">
                            ...
                            <div class="card" aria-label="Announcements">
                                ...
                                <div class="card-body">
                                    <div class="announcements-list">
                                        ...
                                        try {
                                            $stmt = $pdo->query('SELECT * FROM usg_announcements ORDER BY created_at DESC LIMIT 5');
                ...
                <section id="viewAnnouncementsSection" class="section-container d-none">
                    <div class="row">
                        <div class="col-12">
                            <div class="card mt-4 mb-4">
                                <div class="card-header bg-secondary text-white d-flex justify-content-between align-items-center">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-striped table-hover" id="announcementsTable">
                                            ...
                                            <tbody>
                                                <?php
                                                try {
                                                    $stmt = $pdo->query('SELECT * FROM usg_announcements ORDER BY created_at DESC');
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-striped table-hover" id="feedbackTable">
                                            ...
                                            <tbody>
                                                ...
                                                try {
                                                    $stmt = $pdo->query('SELECT * FROM usg_feedbk ORDER BY feed_id DESC');
