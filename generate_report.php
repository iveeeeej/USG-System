...
switch ($reportType) {
    case 'events':
        $query = "SELECT * FROM usg_events WHERE 1=1 $dateCondition ORDER BY startdate DESC";
        $filename = 'events_report.csv';
        $headers = ['ID', 'Event Name', 'Start Date', 'End Date', 'Description'];
        break;

    case 'attendance':
        $query = "SELECT a.*, e.eventname FROM usg_attendance a 
                 JOIN usg_events e ON a.event_id = e.id 
    ...
    case 'payments':
        $query = "SELECT * FROM usg_pay WHERE 1=1 $dateCondition ORDER BY pay_startdate DESC";
        $filename = 'payments_report.csv';
        $headers = ['ID', 'Payment Name', 'Amount', 'Start Date', 'End Date', 'Description'];
        break;

    case 'lost_and_found':
        $query = "SELECT * FROM usg_lst_fnd WHERE 1=1 $dateCondition ORDER BY date_found DESC";
        $filename = 'lost_and_found_report.csv';
        $headers = ['ID', 'Item Name', 'Category', 'Date Found', 'Location', 'Status', 'Description'];
        break;

    case 'feedback':
        $query = "SELECT * FROM usg_feedbk ORDER BY feed_id DESC";
